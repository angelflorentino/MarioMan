...
var p5 = new p5();
var standarSize = 40;
var audio = new Audio('temaMario.mp3');
audio.loop = true;
...
function preload() {
  ...
  powerImg = loadImage('img/power.png');
  angrypacmanImg = loadImage('img/mario.png');
  enemy1Img = loadImage('img/pig_1.png');
  enemy2Img = loadImage('img/pig_2.png');
  enemy3Img = loadImage('img/pig_3.png');
  enemy1ImgWeak = loadImage('img/pig_1.png');
  enemy2ImgWeak = loadImage('img/pig_2.png');
  enemy3ImgWeak = loadImage('img/pig_3.png');
}
...
function draw() {
  ...
  for(var i = 0; i < activeEnemies.length; i++) {
    ...
    if(angrypacman.instance.enemyPackmanColission(activeEnemies[i])) {
      if(activeEnemies[i].isWeak === true) {
        ...
        if(confirm("*** PERDIÓ - Volver al menu ***")){
  ...
  if(foods.length <= 0) {
    if(confirm("*** GANÓ - Volver al menu ***")){
      window.location.replace("index.html");
    }else{
      window.location.reload();
    }
...
this.showPac = function() {
  p5.image(this.image, this.x, this.y, standarSize, standarSize);
}
